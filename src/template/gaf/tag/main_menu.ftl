...
<#if menu.checkRoles(user.getRoles())>
    <#if (menu.subMenues?? && menu.subMenues?size>0)>
        <li>
            ...
            <a class="icon icon-right-arrow" href="#">${label}</a>
            <div class="mp-level">
                <h2 class="icon icon-folder-open">${label}</h2>
                <ul>
        ...
        <#if menu.active>
            ...
            <#assign label=menu.label!'no_label'/>
            <#if menu.domain??>
                <#assign link=tag.calcAction('nav_'+menu.domain.name, null, null)/>
                <#assign label=tag.getText('menu.nav_'+menu.domain.name, label)/>
            <#elseif menu.action??>
            ...
            <a class="icon <#if menu.icon??>icon-${menu.icon}<#else>icon-radio-unchecked</#if> mp-menu-item" href="${link}">${label}</a>
            </li>
        </#if>
    </#if>
</#if>
